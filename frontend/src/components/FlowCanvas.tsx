...
const BeatNode = ({ data }: any) => {
  return (
    <div className="bg-zinc-950/90 border border-zinc-800 rounded-xl p-3.5 w-60 shadow-2xl glass-panel text-zinc-300 relative">
      ...
      <div className="flex items-center gap-1.5 mt-2 border-t border-zinc-900/60 pt-2 text-[9px] font-mono select-none">
        ...
        <span className="ml-auto text-zinc-500">⏱ {data.duration}s</span>
      </div>
...
export default function FlowCanvas({ shots, mediaUrl }: FlowCanvasProps) {
    ...
    const generatedNodes = shots.map((s, idx) => {
      return {
        ...
        data: {
          ...
          duration: s.camera.duration.toFixed(1)
        }
      };
    });
